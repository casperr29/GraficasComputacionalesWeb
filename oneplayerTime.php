...
<body>
  ...
  <script type="text/javascript">
    ...
    var car_velocity = 120.0;
    var obj_velocity = car_velocity * 0.7;
    var track_size = 60;
    ...
    function loadModels() {
      ...
      fbxLoader.load("assets/ARROW_ED.fbx", (speed_ramp) => {
        const [x, y, z] = Array(3)
          .fill()
          .map(() => THREE.Math.randFloatSpread(100));
        speed_ramp.position.set(x, 1, obj_z_distance);
        // speed_ramp.rotation.x += 0.01;
        // speed_ramp.rotation.y += 0.01;
        speed_ramp.scale.set(10, 10, 10);

        addCollisionBox("speed_ramp", speed_ramp);

        scene.add(speed_ramp);
        collisionItems.push(speed_ramp.collBox);

        speed_ramp.name = "speed_ramp";
      });

      fbxLoader.load("assets/clocknowall_ED.fbx", (clock_i) => {
        const [x, y, z] = Array(3)
          .fill()
          .map(() => THREE.Math.randFloatSpread(100));
        clock_i.position.set(x, 1, obj_z_distance);

        addCollisionBox("clock", clock_i);


        scene.add(clock_i);
        collisionItems.push(clock_i.collBox);

        clock_i.name = "clock";

        clock_i.scale.set(10, 10, 20);
      });
      ...
      for (var i = 0; i < camera.rays.length; i++) {
        ...
        if (
          ...
          if (!collisionIt[0].object.taken) {
            ...
            if (collisionIt[0].object.itemType == "speed_ramp") {
    ...
    function animate() {
      ...
      if (clock.getElapsedTime() < 3)
        ...
        player_score = clock.getElapsedTime() - 3 - player_clocks;

      var min = Math.floor(player_score / 60);
      var sec = Math.floor(player_score % 60);

      var format_score = (min <= 9 ? "0" + min : min) + ":" + (sec <= 9 ? "0" + sec : sec);

      $("#playerScore").html(format_score);
      ...
      if (clock.running && clock.getElapsedTime() > 3) {
        ...
        if (distanceLeft <= 0) {
          ...
          $("#modalPlayerScore").html(format_score);
        ...
        ///////MOVIMIENTOS DE RAMPAS Y OBSTACULOS //////////

        var speed_ramp = scene.getObjectByName("speed_ramp");
        speed_ramp.position.y -= obj_velocity * deltaTime;
        if (speed_ramp.position.y <= -80) {
          speed_ramp.position.y = 1;
          const [x, y, z] = Array(3)
            .fill()
            .map(() => THREE.Math.randFloatSpread(100));
          speed_ramp.position.set(x, 10, 1);
        }

        var clock_i = scene.getObjectByName("clock");
        clock_i.position.y -= obj_velocity * deltaTime;
        if (clock_i.position.y <= -80) {
          clock_i.position.y = 1;
          const [x, y, z] = Array(3)
            .fill()
            .map(() => THREE.Math.randFloatSpread(100));
          clock_i.position.set(x, 3, 1);
        }
  ...
  <div class="overlay d-flex justify-content-center align-items-center" id="modalSubmitScore">
    <div class="container-neon d-flex flex-column justify-content-center align-items-center position-relative">
      ...
      <form action="#modalSubmitScore" method="post">
        ...
        <input hidden type="text" name="tableIndex" id="tableIndex" value=1 />
        <button id="submitScoreButton" type="submit" class="btn-neon fs-1" onclick="hideButton('submitScoreButton')">
          <span id="span1"></span>
          <span id="span2"></span>
          <span id="span3"></span>
          <span id="span4"></span>
      ...
      <a href="#" class="btn-neon fs-2" onclick="togglePause('oneplayerTime')">
        <span id="span1"></span>
        <span id="span2"></span>
        <span id="span3"></span>
        <span id="span4"></span>
